#!/usr/bin/env python
"""
    Small example of a library implementing interaction with MICEX ISS server.
...
requests = {'history_secs': 'http://www.micex.ru/iss/history/engines/%(engine)s/markets/%(market)s/boards/%(board)s/securities.json?date=%(date)s'}


class Config:
    def __init__(self, user='', password='', proxy_url='', debug_level=0):
        """ Container for all the configuration options:
            user: username in MICEX Passport to access real-time data and history
        ...
        self.auth_url = "https://passport.micex.ru/authenticate"
...
class MicexAuth:
    ...
    def auth(self):
        ...
        # we only need a cookie with Micex Passport (certificate)
        self.passport = None
